...
type DetResult = ((f32, f32, f32, f32, u32, f32), Props);
...
pub fn nms_cpu(boxes: &mut Vec<DetResult>, iou_threshold: f32, filter_out_boxes: &mut Vec<DetResult>) {
    ...
    while !idx.is_empty() {
        ...
        let good_xmin = t_good_box.0.0;
        let good_ymin = t_good_box.0.1;
        let good_xmax = t_good_box.0.2;
        let good_ymax = t_good_box.0.3;
        let good_width = good_xmax - good_xmin;
        let good_height = good_ymax - good_ymin;

        for tmp_i in tmp {
            let tt_box = &boxes[tmp_i];
            let temp_xmin = tt_box.0.0;
            let temp_ymin = tt_box.0.1;
            let temp_xmax = tt_box.0.2;
            let temp_ymax = tt_box.0.3;
