...
pub fn preprocess_fast(
    ...
    (0..new_height).into_par_iter().for_each(|y| {
        ...
        for x in 0..new_width {
            ...
            unsafe {
                *dst_ptr.add(dst_index + offset_channel2) = (*src_ptr.add(src_index + 0) as f32 - mean_val[0]) / std_val[0];
                *dst_ptr.add(dst_index + offset_channel1) = (*src_ptr.add(src_index + 1) as f32 - mean_val[1]) / std_val[1];
                *dst_ptr.add(dst_index + offset_channel0) = (*src_ptr.add(src_index + 2) as f32 - mean_val[2]) / std_val[2];
            }
        }
    });
